document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleCalculation() {
      ...
      // Calculate TSQFT (SQFT x quantity)
      let tsqft = 0;
      if (sqft > 0 && quantity > 0) {
        tsqft = sqft * quantity;
        tsqftInput.value = tsqft.toFixed(2);
      }
      
      // Calculate amount (TSQFT x rate)
      let amount = 0;
      if (tsqft > 0 && rate > 0) {
        amount = tsqft * rate;
    ...
    document.getElementById('downloadPDF').addEventListener('click', function() {
      ...
      // Clone the document for PDF generation
      ...
      html2canvas(content, {
        ...
        // Add new pages if content flows to multiple pages
        while (heightLeft >= 0) {
    ...
    document.getElementById('downloadExcel').addEventListener('click', function() {
      ...
      const companyWS = XLSX.utils.aoa_to_sheet([
        ...
        ["Validity", document.getElementById('validityPeriod').value + " days"],
        [""],
  ...
  // Replace the existing downloadPDF event listener with this improved version
document.getElementById('downloadPDF').addEventListener('click', function() {
    // Validate form
    if (!document.getElementById('customerName').value.trim()) {
      toastr.error('Please enter customer name');
  ...
  function populatePdfTemplate() {
    ...
    document.getElementById('pdf-validity').textContent = document.getElementById('validityPeriod').value + ' days';
  ...
  function handleCalculation() {
    ...
    // Calculate SQFT (width x length in feet)
  ...
  function populatePdfTemplate() {
    ...
    document.querySelectorAll('#itemsTable tbody tr').forEach((row, index) => {
      ...
      const lengthRemainder = lengthInches % 12;
      
      newRow.innerHTML = `
        <td>${index + 1}</td>
        <td>${particulars}</td>
        <td>${quantity}</td>
        <td>${widthInches}" (${widthFeet}' ${widthRemainder}")</td>
        <td>${lengthInches}" (${lengthFeet}' ${lengthRemainder}")</td>
        <td>${sqft}</td>
        <td>${tsqft}</td>
        <td>₹${rate}</td>
        <td>₹${amount}</td>
